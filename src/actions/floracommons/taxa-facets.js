...
export function taxaFacetsQuery (morphFacets, simpleFacets, pageNo=0, perPage=100) {
  ...
  WHERE {
    ...
    ${ addIf(simpleFacets?.family?.length, [simpleFacets], (sf) => `VALUES ?families {${sf?.family.map(v => `<${fcEndpoint.instance}/entity/${v}>`).join(' ')}}`)}
    ${ addIf(simpleFacets?.rank?.length, [simpleFacets], (sf) =>`VALUES ?ranks {${sf?.rank.map(v => `<${fcEndpoint.instance}/entity/${v}>`).join(' ')}}`)}
  ...
  return {
    ...
    fetch: async function (options = {}) {
      ...
      return await fetch(`${url}${opts.breakCache ? '&nocache=true' : ''}`).then(async response => {
